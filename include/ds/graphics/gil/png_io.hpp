...
namespace ds { namespace graphics { namespace gil {
      ...
      {
      protected:
        static void error_handler(png_structp ptr, png_const_charp msg)
        {
          //png_io * self((png_reader*)png_get_error_ptr(ptr));
          dsE("png: "<<msg);
        }

        static void warn_handler(png_structp ptr, png_const_charp msg)
        {
          //png_io * self((png_reader*)png_get_error_ptr(ptr));
          dsL("png: "<<msg);
        }
      ...
      struct png_reader : png_io
      {
        static bool check( std::istream & is )
        {
          ...
          if ( is.gcount() != sigSize )
            { dsE("png_reader::check: failed to read file"); return false; }

          if ( png_sig_cmp((png_bytep)buf, (png_size_t)0, sigSize) != 0 )
            { dsE("png_reader::check: invalid png file"); return false; }
        ...
        {
          const std::size_t sigSize( boost::gil::detail::PNG_BYTES_TO_CHECK );
          if ( doCheck && !check(_is) )
            { dsE("png_reader::check: bad png stream"); return; }

          _png = png_create_read_struct(PNG_LIBPNG_VER_STRING, this, error_handler, warn_handler);
          if ( _png == NULL )
            { dsE("failed png_create_read_struct(): "<<PNG_LIBPNG_VER_STRING); return; }

          _info = png_create_info_struct( _png );
          if ( _info == NULL )
            {
              png_destroy_read_struct(&_png, NULL, NULL);
              dsE("failed png_create_info_struct()"); return;
            }
        ...
        void read(const boost::gil::image_view<Locator<Iterator<boost::gil::packed_pixel<VT,PF,Layout>*> > > & view)
        {
          ...
          boost::shared_ptr<char> s = ds::cxxabi::demangle(typeid(view).name());
          std::clog<<__FILE__<<":"<<__LINE__<<":TODO: read "<<s.get()<<std::endl;
        }
      ...
      struct png_writer : png_io
      {
          ...
          , _os     ( os )
        {
          using boost::gil::io_error;
          using boost::gil::io_error_if;
          _png = png_create_write_struct(PNG_LIBPNG_VER_STRING, this, error_handler, warn_handler);
        ...
        void write(const boost::gil::image_view<Locator<Iterator<boost::gil::packed_pixel<VT,PF,Layout>*> > > & view)
        {
          ...
          boost::shared_ptr<char> s = ds::cxxabi::demangle(typeid(view).name());
          std::clog<<__FILE__<<":"<<__LINE__<<":TODO: write "<<s.get()<<std::endl;
        }
